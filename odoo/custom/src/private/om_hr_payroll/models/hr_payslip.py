...
class HrPayslip(models.Model):
    ...
    # MIGRATION: Replaced contract_id with version_id
    ...
    payslip_run_id = fields.Many2one(
        "hr.payslip.run", string="Payslip Batches", copy=False
    )
    ...
    def refund_sheet(self):
        ...
        return {
            ...
            "views": [
                (list_view_ref and list_view_ref.id or False, "list"),
    ...
    # MIGRATION: Renamed get_contract to get_versions and targeted hr.version
    @api.model
    def get_versions(self, employee, date_from, date_to):
        ...
        @return: returns the ids of all the versions for the given employee that need to be considered for the given dates
        """
        # A version is valid if it overlaps with the payslip period
        # Logic: (StartA <= EndB) and (EndA >= StartB)
        # UPDATED: using contract_date_start and contract_date_end
        clause = [
        ...
        return self.env["hr.version"].search(clause).ids
    ...
    def compute_sheet(self):
        for payslip in self:
            ...
            # MIGRATION: Use version_id instead of contract_id
            version_ids = payslip.version_id.ids or self.get_versions(
            ...
            if not version_ids:
                raise ValidationError(
                    _(
                        "No running version/contract found for the employee: %s or no version in the given period"
                        % payslip.employee_id.name
                    )
                )

            # MIGRATION: Pass version_ids
            lines = [
                (0, 0, line)
                for line in self._get_payslip_lines(version_ids, payslip.id)
            ]
    ...
    def get_worked_day_lines(self, versions, date_from, date_to):
        ...
        """
        res = []
        # MIGRATION: Check if resource_calendar is on version or employee.
        # Falling back to employee's calendar if version doesn't have specific one.
        for version in versions:
            # Assumes hr.version might have a structure or calendar, otherwise uses Employee's
            calendar = version.employee_id.resource_calendar_id
            if not calendar:
                continue
            ...
            day_leave_intervals = version.employee_id.list_leaves(
    ...
    def get_inputs(self, versions, date_from, date_to):
        ...
        # MIGRATION: Assuming hr.version has a structure_id field like hr.contract did
        # If structure is on Employee, this needs adjustment
        structure_ids = versions.mapped("struct_id").ids
        if not structure_ids:
            # Fallback if structure is defined on Employee
    ...
    def _get_payslip_lines(self, version_ids, payslip_id):
        # MIGRATION: changed arg name from contract_ids to version_ids
        ...
        # MIGRATION: browse hr.version instead of hr.contract
        versions = self.env["hr.version"].browse(version_ids)

        if len(versions) == 1 and payslip.struct_id:
            structure_ids = list(set(payslip.struct_id._get_parent_structure().ids))
        else:
            # Assuming hr.version has get_all_structures or structure_id
            # If structure is on Employee, logic changes slightly
            structure_ids = (
                versions.mapped("struct_id").ids
                or versions.mapped("employee_id.struct_id").ids
            )
        ...
        for version in versions:
            ...
            # We pass 'contract': version to the dict.
            # This allows existing Salary Rules that use "contract.wage" to work if hr.version has a wage field.
            ...
            for rule in sorted_rules:
                ...
                if rule._satisfy_condition(localdict) and rule.id not in blacklist:
                    ...
                    # MIGRATION: Use company from employee or version
                    ...
                    result_dict[key] = {
                        ...
                        "version_id": version.id,  # MIGRATION
                        "name": rule.name,
                        "code": rule.code,
    ...
    def onchange_employee_id(
        ...
        res["value"].update({"version_id": contract.id})
    ...
    def onchange_employee(self):
        ...
        # Attempt to get structure from version, fallback to employee
        if hasattr(self.version_id, "struct_id") and self.version_id.struct_id:
            self.struct_id = self.version_id.struct_id
        elif employee.struct_id:
...
class HrPayslipLine(models.Model):
    ...
    _order = "version_id, sequence"  # MIGRATION
    ...
    version_id = fields.Many2one(
        "hr.version", string="Version/Contract", required=True, index=True
    )
        ...
        for values in vals_list:
            if "employee_id" not in values or "version_id" not in values:
                ...
                values["version_id"] = (
                    ...
                    or payslip.version_id
                    and payslip.version_id.id
                )
                if not values["version_id"]:
                    # Relaxed error for testing, but ideally this should be enforced
                    pass
...
#         # A version is valid if it overlaps with the payslip period
#         # Logic: (StartA <= EndB) and (EndA >= StartB)
#         # Assuming hr.version has date_start and date_end fields
#         clause = [
#             ('employee_id', '=', employee.id),
#             ('date_start', '<=', date_to),
#             '|',
...
#                           self.get_versions(payslip.employee_id, payslip.date_from, payslip.date_to)
#
#             if not version_ids:
#                 raise ValidationError(
